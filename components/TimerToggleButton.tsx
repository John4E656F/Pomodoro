...
export const TimerToggleButton: React.FC<Props> = ({ startCountDownHandler, stopCountDownHandler, isTimerRunning }) => {
  const toggleTimer = () => {
    isTimerRunning ? stopCountDownHandler() : startCountDownHandler();
  };

  return (
    <Pressable onPress={toggleTimer} style={({ pressed }) => [{ opacity: pressed ? 0.6 : 1 }]}>
